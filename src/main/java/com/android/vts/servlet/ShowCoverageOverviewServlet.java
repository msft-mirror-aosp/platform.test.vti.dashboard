...
/** Represents the servlet that is invoked on loading the coverage overview
 * page. */
public class ShowCoverageOverviewServlet extends BaseServlet {
  private static final String COVERAGE_OVERVIEW_JSP =
      "WEB-INF/jsp/show_coverage_overview.jsp";
  ...
  @Override
  public void doGetHandler(HttpServletRequest request,
    ...
    for (VtsReportMessage.TestCaseResult r :
         VtsReportMessage.TestCaseResult.values()) {
      resultNames.add(r.name());
    }

    Map<String, Object> parameterMap = request.getParameterMap();
    Query.Filter userTestFilter = FilterUtil.getUserTestFilter(parameterMap);
    Query.Filter userDeviceFilter =
        FilterUtil.getUserDeviceFilter(parameterMap);

    List<JsonObject> testRunObjects = new ArrayList<>();

    Query.Filter testFilter = new Query.FilterPredicate(
        TestRunEntity.HAS_COVERAGE, Query.FilterOperator.EQUAL, true);
    testFilter = Query.CompositeFilterOperator.and(
        testFilter, FilterUtil.getTestTypeFilter(showPresubmit, showPostsubmit,
                                                 unfiltered));
    if (userTestFilter != null) {
      testFilter =
          Query.CompositeFilterOperator.and(testFilter, userTestFilter);
    }
    int coveredLines = 0;
      ...
      if (userTestFilter == null && userDeviceFilter == null) {
        ...
        for (Entity testRun :
          ...
          TestRunMetadata metadata =
              new TestRunMetadata(key.getName(), testRunEntity);
          testRunObjects.add(metadata.toJson());
          coveredLines += testRunEntity.coveredLineCount;
          uncoveredLines +=
              testRunEntity.totalLineCount - testRunEntity.coveredLineCount;
        ...
        if (userTestFilter != null) {
          testFilter =
              Query.CompositeFilterOperator.and(userTestFilter, testFilter);
        }
        List<Key> gets = FilterUtil.getMatchingKeys(
            key, TestRunEntity.KIND, testFilter, userDeviceFilter,
            Query.SortDirection.DESCENDING, 1);
    ...
    testStats[VtsReportMessage.TestCaseResult.TEST_CASE_RESULT_PASS
                  .getNumber()] = passCount;
    testStats[VtsReportMessage.TestCaseResult.TEST_CASE_RESULT_FAIL
                  .getNumber()] = failCount;
    ...
    request.setAttribute("branches",
                         new Gson().toJson(DatastoreHelper.getAllBranches()));
    request.setAttribute(
        "devices", new Gson().toJson(DatastoreHelper.getAllBuildFlavors()));
